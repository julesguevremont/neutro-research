...
| Category | Count |
|----------|-------|
| Total .py files in `modules/` | 226 (16 deleted in V11.77) |
| Subdirectory modules (`daemon/`, `senses/`) | 25 |
| **Actually imported** | ~53 unique (V11.75+3, V11.77+5) |
| **Dead code candidates** | ~178 files (75%) |
...
| `curiosity_engine` | 1 | Developmental curiosity tracking (V11.77) |
| `code_sandbox` | 1 | Safe Python code execution (V11.77) |
| `analogy_reasoning` | 1 | Analogy suggestions (V11.77) |
| `counter_argument` | 1 | Critical thinking prompts (V11.77) |
| `meta_reasoning` | 1 | Confidence evaluation (V11.77) |

*Plus 21 more modules imported exactly once*

### DEAD CODE CANDIDATES (Never Imported)

These ~178 files exist in `modules/` but are **never imported** by any active code (16 deprecated files deleted in V11.77):
...
### V11.77: Reasoning Enhancement Integration

```
daemon_runner.py query processing:
    │
    ├── CuriosityEngine (modules/curiosity_engine.py)
    │   ├── calculate_curiosity_level(query)
    │   ├── register_learning_event(query)
    │   └── Tracks developmental stages (baby → toddler → child)
    │
    ├── CodeSandbox (modules/code_sandbox.py)
    │   ├── execute_python(code) via /execute API endpoint
    │   ├── Safe execution with timeout
    │   └── Returns {success, output, error}
    │
    ├── AnalogyReasoning (modules/analogy_reasoning.py)
    │   ├── should_use_analogy(query)
    │   ├── suggest_analogy(query, concepts)
    │   └── add_analogy_prompt(base_prompt)
    │
    ├── CounterArgumentReasoning (modules/counter_argument.py)
    │   ├── should_consider_counter(query)
    │   └── add_counter_prompt(base_prompt)
    │
    └── MetaReasoning (modules/meta_reasoning.py)
        ├── add_self_check(base_prompt)
        └── evaluate_confidence(response_length, query_complexity)
```

**Key integration points in daemon_runner.py:**
- Lines 72-77: V11.77 imports for all 5 reasoning modules
- Lines 162-167: Module instances in DaemonState class
- Lines 310-318: Initialization in initialize_neutro()
- Lines 861-872: Curiosity and meta-reasoning tracking per query
- Lines 1027-1055: /execute API endpoint for CodeSandbox

**What this enables:**
- Curiosity-driven learning with developmental stages
- Safe Python code execution via API
- Analogy-based explanations for complex concepts
- Critical thinking prompts for opinion questions
- Confidence evaluation per response
...
| Claim | Reality |
|-------|---------|
| 44 integrated modules | ~53 actually imported (V11.77) |
| All modules active | ~178 files never imported |
...
*This document represents the ACTUAL state of NEUTRO as of V11.77, not aspirational documentation.*
